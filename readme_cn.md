...
+ 协程主动让出CPU，没有 “抢占式多任务” 一说。您没看错，“抢占式多任务” 根本不是啥优势，而是**多线程最大的缺点**，是**bug之源**，不知道多少人意识到这一点了？主动让出CPU的协程，可以带来更好的CPU利用率，更多的并发任务数。这也是近年来，不管C++, C#, nodejs, java, php各式语言，都开始引入协程的原因。
+ 协程比任何的所谓 “实时操作系统” 更实时。
+ 协程可以用 __await__ 标注哪个函数运行的时候，CPU可能会被切换到其他任务，多任务间共享变量无比安全。这点也是线程不能比的。

s_task协程库，更是打造了全平台兼容的协程支持环境，从高端linux服务器、windows/apple桌面，到安卓ndk，到stm32、arduino等各式嵌入式无操作系统环境都能支持，有些芯片的运行内存甚至可以低至1K大小。（[参考兼容性列表](#compatibility)）
